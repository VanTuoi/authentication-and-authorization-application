...
export default function setNavPills(): void {
  ...
  function initNavs(): void {
    total.forEach((item: any) => {
      ...
      item.onmouseover = function (event: MouseEvent) {
        ...
        if (li) {
          const nodes = Array.from(li.closest("ul")!.children) as HTMLElement[];
          const index = nodes.indexOf(li) + 1;
          (item.querySelector(`li:nth-child(${index}) .nav-link`) as HTMLElement).onclick = function () {
            moving_div = item.querySelector(".moving-tab") as HTMLElement;
            let sum = 0;
  ...
  window.addEventListener("resize", () => {
    total.forEach((item) => {
      ...
      const li = item.querySelector(".nav-link.active")!.parentElement as HTMLElement;
    ...
    if (window.innerWidth < 991) {
      total.forEach((item) => {
        if (!item.classList.contains("flex-column")) {
          ...
          const moving_div = document.querySelector(".moving-tab") as HTMLElement;
          moving_div.style.width = `${(item.querySelector("li:nth-child(1)") as HTMLElement).offsetWidth}px`;
          moving_div.style.transform = `translate3d(0px, ${sum}px, 0px)`;
        }
      });
